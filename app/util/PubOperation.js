...
Ext.define('iTenants.util.PubOperation', {
	...
	showLoadMask : function() {
		...
		iTenants.ux.CustomLoading.show({
...
//		iTenants.ux.CustomLoading.hide();
//		iTenants.ux.CustomLoading.showTip({
...
//		});
	...
	initLanguage : function() {
		if(this.isGoogleChrome()){
			...
			iTenants.ux.Manager.init();
	        
	    	// Destroy the #appLoadingIndicator element
	        Ext.fly('appLoadingIndicator').destroy();
	        // Initialize the main view
	        Ext.Viewport.add(Ext.create('iTenants.view.Main'));
//	        Ext.Viewport.add(Ext.create('iTenants.view.layoutTest'));
	        Ext.Function.defer(function(){
				// 推送入口
	...
	loginPub : function(callbackFn,isCheck){
		...
		var resultFun = function(responseText) {
			...
			if(resJSON.Success){
				...
				Ext.Function.defer(function(){iTenants.util.PubOperation.showTips('failureMsg','failure');},500);
			}
        };
	
		var failureFun = function() {
			callbackFn(Global.userToken);
			Ext.Function.defer(function(){iTenants.util.PubOperation.showTips('requestErrorMsg','failure');},2000);
		};
		// 调用ajax
		iTenants.util.PubOperation.ajaxFun(url, params, resultFun, failureFun, true, false , 'login_iTenants');	
	},
	/**
	 * Ajax数据加载
	 * 
	...
	ajaxFun : function(url, params, resultFun, failureFun, isCheck,loadFormCache, key,requestFun) {
		try {
			//离线缓存
			if(loadFormCache){
				iTenants.util.PubOperation.offlineAccess(key,null,resultFun);
			}
			
			// 判断网络
			if (!iTenants.util.PubOperation.checkConnectionStatus()) {
				if (isCheck == true) {
					iTenants.util.PubOperation.showTips('nonetworkconnection', 'normal');
				}
				return;
			}
	...
	pubListLoad : function(store, params, isCheck, loadFormCache, key, callBackFun) {
		try{
			...
			// 离线加载
			if(loadFormCache){
				iTenants.util.PubOperation.offlineAccess(key,store);
			}
			// 判断网络
			if(!iTenants.util.PubOperation.checkConnectionStatus()){
				if (isCheck == true) {
			...
			if (params) {
				...
				store.loadPage(1, {
					callback : function(record, operation, success) {
						if (success) {
							// 检查返回数据是否为空/请求成功返回错误信息处理
							iTenants.util.PubOperation.emptyDataOperation(record,
									operation.getResponse(), success);
						}else{
							iTenants.util.PubOperation.showTips('requestErrorMsg',
							'failure');
						}
					},
					scope : this
	...
	checkConnectionStatus : function() {
		...
		if (!this.isGoogleChrome()) {
			...
			if (connectionType == 'Unknownconnection'
				...
				iTenants.util.PubOperation.showTips('nonetworkconnection',
						"normal");
				return false;
			}
		}
		...
		if (success) {
			...
			if (!response.Success) {
				if (response.ErrorCode == -1) {
					iTenants.util.PubOperation.showTips('abnormalMsg', 'failure');
				} else if (response.ErrorCode == -2) {
					iTenants.util.PubOperation.showTips('paramsMsg', 'failure');
				} else if (response.ErrorCode == -3) {
					iTenants.util.PubOperation.showTips('systemErrorMsg',
							'failure');
				} else if (response.ErrorCode == -4) {
					iTenants.util.PubOperation.showTips('permissionsMsg',
							'failure');
				} else if(response.ErrorCode == -8) {
					iTenants.util.PubOperation.showTips('projectStatusMsg',
					'failure');
				}
			} else {
...
//			}
	...
	attsFileOpen : function(src) {
		...
		if (Ext.os.is.Android) {
			iTenants.util.PubOperation.maskMessage('附件加载中，请稍候...');
			iTenants.util.PhoneGapAPI.downloadFileForAndroid(src, {
				overwrite : false,
				open : true
			}, function(res) {
			}, function(error) {
			...
			iTenants.util.PhoneGapAPI.downloadFileForIOS(
	...
	openImage : function(url) {
		if (Ext.os.is.Android) {
			iTenants.util.PhoneGapAPI.openImageForAndroid(url, {
				overwrite : false,
				open : true
			}, function(res) {
			}, function(error) {
			...
			iTenants.util.PhoneGapAPI.openImageForIOS(function() {
			}, function() {
			}, url);
		}
	},
